...
plot_georgia_aoa <- function(train_data, test_data, predictors) {
  ...
  plt <- ggplot2::ggplot(ga_aoa_sf) +
    ...
    ggplot2::theme_void() +
    ggplot2::theme(
      text = ggplot2::element_text(color = "black", size = 14),
...
plot_ga_comparison_map <- function(pred_data, boundaries) {
  ...
  ga_boundary <- boundaries %>% dplyr::filter(NAME == "GA")
  ...
  # 2. Helper Function
  create_map <- function(data, fill_col, title_text) {
    ggplot2::ggplot() +
      # Background
      ...
      ggplot2::theme(
        ...
        # Axis Ticks & Labels (Enabled)
        axis.text = ggplot2::element_text(color = "grey30", size = 9),
        axis.title = ggplot2::element_text(color = "grey30", size = 10),
        
        # Legend Sizing (Increased)
  ...
  # 3. Create & Combine
  p_pred   <- create_map(plot_data, .pred_class, "Predicted")
  p_actual <- create_map(plot_data, forested,    "Actual")
...
plot_ga_error_map <- function(pred_data, boundaries) {
  # 1. Filter for Errors Only & Define Type
  errors <- pred_data %>%
    dplyr::filter(.pred_class != forested) %>%
    dplyr::mutate(
      error_type = dplyr::case_when(
        .pred_class == "Yes" & forested == "No" ~ "False Positive (Phantom Forest)",
        .pred_class == "No" & forested == "Yes" ~ "False Negative (Missed Forest)"
      )
    )
  ...
  # 3. Plot
  ggplot2::ggplot() +
    # Background (Keep it clean/grey)
    ggplot2::geom_sf(data = ga_boundary, fill = "grey98", color = "grey80") +
    
    # The Errors
    ggplot2::geom_sf(data = error_sf, 
                     ggplot2::aes(color = error_type), 
                     size = 0.6, alpha = 0.6) +
    
    # Colors: Orange for Phantom, Purple for Missed
    ggplot2::scale_color_manual(
      name = "Error Type",
      values = c(
        "False Positive (Phantom Forest)" = "#D95F02", # Orange
        "False Negative (Missed Forest)"  = "#7570B3"  # Purple
      )
    ) +
